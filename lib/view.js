...
class TerminusView extends View {
  ...
  static getFocusedTerminal () {
    return TerminusView.focusedTerminal || null
  }
  ...
  displayTerminal () {
    ...
    if (this.ptyProcess.childProcess) {
  ...
  destroy () {
    ...
    if (this.terminal) {
      if (TerminusView.focusedTerminal === this.terminal) { TerminusView.focusedTerminal = null }
  ...
  copy () {
    const rawText = this.terminal ? this.terminal.getSelection() : ''
  ...
  focusTerminal (fromWindowEvent) {
    ...
    this.terminal.focus()
    TerminusView.focusedTerminal = this.terminal
  ...
  resizeTerminalToView () {
    ...
    if (prevCols === cols && prevRows === rows) { return }
  ...
  getDimensions () {
    if (this.terminal && this.fitAddon) {
      this.fitAddon.fit()
      const rowElement = this.terminal.element && this.terminal.element.querySelector('.xterm-rows > div')
      if (rowElement) {
        this.rowHeight = rowElement.getBoundingClientRect().height || this.rowHeight
      }
      return { cols: this.terminal.cols, rows: this.terminal.rows }
    }

    const cols = Math.floor(this.xterm.width() / 9)
    const rows = Math.floor(this.xterm.height() / 20)
    return { cols, rows }
  }
